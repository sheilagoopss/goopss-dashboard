...
const getPackageId = (displayName: string): string => {
  const packageIdEntry = Object.entries(packageTypes).find(
    ([, value]) => value === displayName,
  );

  return packageIdEntry ? packageIdEntry[0] : "";
};
...
export const usePlanTaskRules = () => {
  ...
  const fetchTaskRules = useCallback(async () => {
    ...
    try {
      ...
      const rules = rulesDoc.exists()
        ? (rulesDoc.data() as PlanTaskRules).tasks
        : [];
  ...
  const fetchSections = useCallback(async () => {
    ...
    try {
      const rulesRef = doc(db, "planTaskRules", "default");
      const rulesDoc = await getDoc(rulesRef);
      const sections = rulesDoc.exists()
        ? (rulesDoc.data() as PlanTaskRules).sections
        : [];
  ...
  const handleEdit = useCallback(
    async (editedRule: PlanTaskRule) => {
      setIsLoading(true);
      try {
        const docId = selectedPackage
        ...
        if (rulesDoc.exists()) {
          ...
          message.success(
            `Rule updated in ${selectedPackage || "default"} rules`,
          );
          return updatedTasks;
        }
      } catch (error) {
  ...
  const handleDelete = useCallback(
    async (ruleToDelete: PlanTaskRule) => {
      setIsLoading(true);
      try {
        if (
          !window.confirm(
        ...
        if (rulesDoc.exists()) {
          ...
          message.success(
            `Task deleted from ${selectedPackage || "default"} rules`,
          );
          return updatedTasks;
        }
      } catch (error) {
  ...
  const handleSubmit = useCallback(
    async (values: any, editingRule: PlanTaskRule | null, user: any) => {
      setIsLoading(true);
      try {
        const docId =
          selectedPackage === "default"
        ...
        if (rulesDoc.exists()) {
          ...
          message.success(
            `Task rule ${editingRule ? "updated" : "added"} successfully in ${
              selectedPackage || "default"
            } rules`,
          );
          return updatedTasks;
